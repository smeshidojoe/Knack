...
Галочку автозапуска, поставленную в установщике, программа принимает как есть: на
первом запуске она читает реестр, а не навязывает своё значение.
...
Медиасессию заводят тоже не все: реклама, короткие ролики и часть сборок
Chromium звучат мимо неё. На этот случай вкладка переходит на аудиосессии:
показывает, какое приложение шумит, и заголовок его окна — у браузера это
название вкладки. Управлять оттуда нечем, поэтому кнопки и полоса перемотки
гаснут, а вместо обложки показывается значок программы, которая шумит.

Под обложкой — громкость. Она привязана к источнику: двигается уровень того
приложения, которое играет, — тот самый, что показывает системный микшер, — так
что остальное остаётся как было. Если источника нет, полоса показывает общий
уровень системы. Клик или протяжка по полосе, колесо над ней двигает по 5%, клик
по динамику выключает звук. До ползунка внутри самого плеера дотянуться нельзя:
он живёт на странице, а не в Windows.
...
утащить мышью обратно наружу — в папку, в чат, в любое окно, принимающее файлы.
У музыки и видео в буфер уходит путь к файлу, так его вставляют в чат обычным
`Ctrl+V`.
...
| Инструменты | Автопроверка обновлений, обновление, установка ffmpeg |
| Буфер | Длина истории |
| Переводчик | Движок, ключ DeepL, папка моделей, определение языка |
| Замена символов | Включение, сочетание, возврат буфера |
| Закрепление окна | Включение, сочетание, значок в углу, отпускать ли окна при выходе |
...
В правом верхнем углу активного окна, рядом с кнопками заголовка, едет значок с
канцелярской кнопкой: клик закрепляет, повторный отпускает, перечёркнутая кнопка
значит «окно закреплено». Значок выключается в настройках.

Рабочий стол, панель задач и прочие окна оболочки закрепление пропускает: подняв
их поверх всех, мы накрыли бы ими экран. Окна с правами администратора не
поддаются — то же ограничение, что и у замены символов.
...
Через восемь секунд после старта и дальше раз в два часа Knack тихо спрашивает
GitHub, нет ли версии новее. Найденную показывает своей плашкой в углу экрана, и
плашка висит, пока её не тронут: системные уведомления глушит то «Фокусировка
внимания», то полноэкранный режим, и единственное сообщение о новой версии
терялось бы. Клик по ней — панель выезжает, открываются настройки, колонка
доезжает до строки обновления, и поверх встаёт карточка с полосой. Крестик на
плашке откладывает эту версию, и больше о ней не напоминаем.

Если панель в этот момент открыта, то же предложение появляется в ней:
«Обновить» ставит сразу, «Позже» откладывает.

Пункт «Проверить обновление» в меню трея делает то же самое по требованию и
ставит найденное без вопросов. Как бы установку ни начали, разом идёт только
одна.

Пока идёт установка, панель заперта: она показывает карточку с прогрессом и не
закрывается ни хоткеем, ни из трея, ни уходом курсора — программа вот-вот
подменит собственный exe и перезапустится, прятать её на полпути нечего.

Установка подменяет работающий exe, а его Windows держит заблокированным, поэтому
новый exe распаковывается рядом и запускается с флагом `--apply-update`: он ждёт
выхода старого, копирует себя на его место и запускает. Если запустить его не
дали, ту же работу делает помощник на PowerShell.

Всё это работает только в собранной программе: из исходников подменять нечего.
...
Движение подчиняется системе: выключи «Показывать анимацию в Windows» в
специальных возможностях — панель перестанет выезжать, карточки выплывать, и
останется только проявление. Всё, что всё-таки движется, укладывается в 300 мс и
замедляется к концу, а не к началу: медленное начало придерживает ровно тот
